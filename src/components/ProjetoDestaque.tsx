...
const ProjetoDestaque: React.FC<ProjetoProps> = ({ titulo, descricao, imagem, categoria, saibaMais, invertido}) => {
    ...
    return (
        <div
            className={`flex ${
                invertido ? "flex-col-reverse md:flex-row-reverse" : "flex-col-reverse md:flex-row"
            } justify-center items-center`}
            >
            <div className={`flex flex-col w-96 pl-8 pr-8 lg:pl-0 lg:pr-0 text-justify ${
                invertido ? " md:mr-10 " : ""
                }`}>
                <h1 className={`font-bold ${
                    invertido ? "md:text-end" : "text-start"
                }`}>{titulo}</h1>
                <br />
                <p>{descricao}</p>
                <br />
            ...
            <div className={`${
                invertido ? "m-10 md:ml-0 md:mr-10 md:my-10" : "m-10"
            }`}
            >
                <Image src={imagem} alt={titulo} width={500} height={300} />
            </div>
        </div>
